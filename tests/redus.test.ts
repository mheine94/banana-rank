...
import {
  createUser,
  createUserData as createUserJsonData,
} from "./test.helpers";

const SORT_STRATEGIES = Object.keys(SortStrategy).filter((v) =>
  isNaN(Number(v)),
);
const SELECTION_STRATEGY = Object.keys(SelectionStrategy).filter((v) =>
  isNaN(Number(v)),
);
...
describe("sort strategy reducer set action", () => {
  SORT_STRATEGIES.map(
    (s) => SortStrategy[s as keyof typeof SortStrategy],
...
describe("selection strategy reducer set action", () => {
  SELECTION_STRATEGY.map(
    (s) => SelectionStrategy[s as keyof typeof SelectionStrategy],
...
describe("select leaderboard (exact match)", () => {
  ...
  it(`should return top 10 with selected user when searched is in top ten`, () => {
    ...
    let expectedLeaderBoard = TOP_TEN.map((user) => ({
      user,
      selected: false,
    }));
  ...
  it(`should sort by name when sort strategy is ${SortStrategy[SortStrategy.BY_NAME]}`, () => {
    ...
    let expectedLeaderBoard = testUsers.map((user) => ({
      user,
      selected: false,
    }));
  ...
  it(`should return bottom 10 with selected user when searched is in bottom ten`, () => {
    ...
    let expectedLeaderBoard = BOTTOM_TEN.map((user) => ({
      user,
      selected: false,
    }));
    expectedLeaderBoard[0].selected = true;
