...
export default function LeaderBoard({ leaderBoard } : LeadeBoardProps) {
    ...
    const getRowBackGroundColor = (entry : LeaderBoardEntry) : string | undefined => {
...
return (
 <View>
        ...
        {leaderBoard.slice(from, to).map((entry) => (
          <DataTable.Row style={{ backgroundColor: getRowBackGroundColor(entry)}} key={entry.user.rank} onLongPress={() => copyToClipboard(entry.user.name)}>
            <DataTable.Cell style={styles.rank} >{entry.user.rank}</DataTable.Cell>
